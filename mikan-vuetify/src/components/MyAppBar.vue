<template>
    <v-app-bar app color="red lighten-1" dark>
      ...
      <v-btn icon @click="toggleDark" class="ml-4" :class="['dark-toggle', { 'active': darkMode }]"><v-icon>{{ darkMode ? 'mdi-weather-night' : 'mdi-weather-sunny' }}</v-icon></v-btn>
      <v-btn icon><v-icon>mdi-account-circle</v-icon></v-btn>
      <v-text-field
        flat hide-details placeholder="Search..."
        append-inner-icon="mdi-magnify"
        class="mx-4 search-white" style="max-width:200px"
      />
    </v-app-bar>
  </template>
  <script setup>
